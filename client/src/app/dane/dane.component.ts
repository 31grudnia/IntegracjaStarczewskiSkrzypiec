...
export class DaneComponent implements OnInit {
  ...
  getDane() {
    return this.http
      .get('https://localhost:5003/api/ceny/getCenyBaza/')
      .subscribe((res) => {
        this.dane = res;
      });
  }
  ...
  addDane() {
    return this.http
      .post('https://localhost:5003/api/db/dodajSong', this.model)
      .subscribe((res) => {
        console.log(res);
  ...
  deleteDane() {
    return this.http
      .delete('https://localhost:5003/api/db/usunSong/' + this.idDelete)
      .subscribe(
        (res) => {
          console.log(res);
